<?php defined('LIBRARY') OR die('No direct access allowed.');
/**
 * Input library ported from the Kohana Library. SPT 11/14/11
 *
...
class Input 
{
	...
	{
		// Use XSS clean?
		// self::use_xss_clean = (bool) Kohana::config('core.global_xss_filtering');
		$use_xss_clean = TRUE;
	...
	{
		return self::search_array($_COOKIE, $key, $default, $xss_clean);
	}
	 ...
	 * @return  string
	 */
	public function xss_clean($data, $tool = FALSE)
	{
		if (is_array($data))
		{
	...
	{
		$pattern = (bool) preg_match('/^\pL$/u', 'ñ');
		$chars = $pattern ? '\pL' : 'a-zA-Z';
